...
const getD1ServerOnly = createServerOnlyFn(async (): Promise<D1Database> => {
  const { getD1 } = await import("@/lib/d1");
  return getD1();
});

const getSessionServerOnly = createServerOnlyFn(async () => {
  const [{ getRequest }, { getSessionFromRequest }] = await Promise.all([
...
const normalizeEmailServerOnly = createServerOnlyFn(async (email: string) => {
  const { normalizeEmail } = await import("@/lib/auth");
  return normalizeEmail(email);
});

function serverFn<T>(handler: (data: Input) => Promise<T>, method: "GET" | "POST" = "POST"): any {
  const builder: any = createServerFn({ method })
    .inputValidator((data: unknown) => (data && typeof data === "object" ? data as Input : {}));
  return builder.handler(async ({ data }: { data: Input }) => handler(data));
}
